      ...
      <div className="mx-auto grid w-full max-w-6xl gap-10 lg:grid-cols-[1.05fr,0.95fr] lg:items-center">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[11px] uppercase tracking-[0.35em] text-cyan-200 terminal-chip">
            FundTrace AI Enterprise Gateway
          </div>
          <h1 className="mt-6 font-headline text-3xl uppercase tracking-[0.1em] text-white sm:text-4xl">
            Zero-trust access terminal
          </h1>
          <p className="mt-4 max-w-lg text-sm text-slate-300">
            Select your identity provider, or authenticate with verified email
            credentials. Hardware key enforcement is mandatory for production
            sessions.
          </p>

          <div className="mt-8 grid gap-3">
            {ssoOptions.map((option) => (
              <SignInButton key={option.label}>
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-xs text-slate-300">
            ...
            <p className="mt-2 text-[11px] text-slate-400">
              Insert your registered YubiKey and touch the contact pad when
              prompted.
            </p>
          </div>
        </div>
